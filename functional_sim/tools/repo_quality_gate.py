...
def main() -> int:
    ...
    nested_root = root / "functional_sim"
    if (nested_root / "experiment_config.py").exists():
        root = nested_root
    repo_root = root.parent
    transformers_root = (args.transformers_root or (repo_root.parent / "transformers")).resolve()

    gate = Gate(strict=args.strict)
    gate.ok(f"functional root: {root}")
    gate.ok(f"repository root: {repo_root}")
