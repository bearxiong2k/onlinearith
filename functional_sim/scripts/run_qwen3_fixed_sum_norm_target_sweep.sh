...
prepare_calibration() {
  ...
  while IFS= read -r task; do
    ...
    if ! run_step "$model_key" "$snr" "fixed_sum_${suffix}" "$partial" "$log" \
      env CUDA_VISIBLE_DEVICES="$ARTIFACT_GPU" "$PYTHON" "$FUNCTIONAL_ROOT/calibrate.py" \
  ...
  run_step "$model_key" "$snr" "fixed_sum" "$final_json" "$merge_log" \
    "$PYTHON" "$FUNCTIONAL_ROOT/tools/merge_msd_calibrations.py" "${inputs[@]}" --output "$final_json"
}
...
run_ppl_probe() {
  ...
  run_step "$model_key" "$snr" "ppl_util_fig5" "$out" "$log" \
    env CUDA_VISIBLE_DEVICES="$PPL_GPUS" "$PYTHON" "$FUNCTIONAL_ROOT/ppltest.py" \
      --model-path "$model_path" \
      --setup 6 \
      --calibration "$cal" \
      --stats lite \
