...
if [[ "${BACKGROUND:-0}" == "1" && "${QWEN_SPARSITY_NORM_SWEEP_CHILD:-0}" != "1" ]]; then
  ...
  echo "[launched] ETA monitor: $PYTHON $SCRIPT_DIR/monitor_qwen3_sweep_eta.py --log-root $LOG_ROOT --watch 60"
  exit 0
fi
...
run_wanda_calibration_point() {
  ...
  run_logged wanda "$model_key" "$point" calibration "$mask" "$log_dir/calibration.log" \
    env CUDA_VISIBLE_DEVICES="$gpu" "$PYTHON" "$FUNCTIONAL_ROOT/wanda_base/calibrate_base.py" \
...
run_wanda_ppl_point() {
  ...
  run_logged wanda "$model_key" "$point" ppl_limit "$out" "$log_dir/ppl_limit${LIMIT_SAMPLES}.log" \
    env CUDA_VISIBLE_DEVICES="$GPUS" "$PYTHON" "$FUNCTIONAL_ROOT/wanda_base/ppl_batch_base.py" \
      --model-path "$model_path" \
      --only 1 \
      -n "$n" \
      -m "$m" \
...
run_act_point() {
  ...
  run_logged act "$model_key" "$point" ppl_limit "$out" "$log_dir/ppl_limit${LIMIT_SAMPLES}.log" \
    env CUDA_VISIBLE_DEVICES="$GPUS" "$PYTHON" "$FUNCTIONAL_ROOT/act_base/ppl_batch_base_act.py" \
      --model-path "$model_path" \
      --only 1 \
      -n "$n" \
      -m "$m" \
...
write_summary() {
  "$PYTHON" "$SCRIPT_DIR/summarize_qwen3_sparsity_norm_sweep300.py" \
    --root "$SWEEP_ROOT" \
